...
<form method="POST" action="{{ route('laba-rugi.update', $item) }}">
    @csrf
    @method('PUT')
    <div class="panel">
        <div class="panel-icon-row">
            <div class="panel-icon"><svg class="icon"><use href="#ic-edit"/></svg></div>
            <span>Perubahan akan langsung memengaruhi laporan periode {{ $item->period_label }}.</span>
        </div>
        @include('laba-rugi._form')
        <div class="panel-actions">
            <button type="button" class="btn btn-danger-ghost" onclick="document.getElementById('deleteForm').submit()">Hapus Pos</button>
...
<form method="POST" action="{{ route('laba-rugi.destroy', $item) }}" id="deleteForm" onsubmit="return confirm('Hapus pos laba rugi ini?')" style="display:none;">
    @csrf
    @method('DELETE')
</form>
</x-app-layout>