...
<style>
    ...
    .panel::before{ content:''; position:absolute; top:0; left:0; width:4px; height:100%; background:var(--info); }
    .panel-icon-row{ display:flex; align-items:center; gap:10px; margin-bottom:24px; }
    .panel-icon{ width:38px; height:38px; border-radius:10px; background:rgba(var(--info-rgb),0.12); color:var(--info); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
    ...
    .panel-actions{ margin-top:32px; padding-top:26px; border-top:1px solid var(--border); display:flex; justify-content:space-between; gap:10px; }
...
<div class="page-head">
    ...
    <p>Perbarui transaksi "{{ $item->description }}".</p>
</div>

<form method="POST" action="{{ route('ledger.update', $item) }}">
    @csrf
    @method('PUT')
    <div class="panel">
        <div class="panel-icon-row">
            <div class="panel-icon"><svg class="icon"><use href="#ic-edit"/></svg></div>
            <span>Perubahan akan memengaruhi saldo berjalan akun {{ $item->account_name }}.</span>
        </div>
        @include('ledger._form')
        <div class="panel-actions">
            <button type="button" class="btn btn-danger-ghost" onclick="document.getElementById('deleteFormLedger').submit()">Hapus Transaksi</button>
...
<form method="POST" action="{{ route('ledger.destroy', $item) }}" id="deleteFormLedger" onsubmit="return confirm('Hapus transaksi ini?')" style="display:none;">
    @csrf
    @method('DELETE')
</form>
</x-app-layout>