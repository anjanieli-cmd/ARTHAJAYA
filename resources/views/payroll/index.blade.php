<x-app-layout>
    ...
    @php
        ...
        $payrolls = $payrolls ?? [
            ['id' => 1, 'employee' => 'Budi Santoso',      'position' => 'Pengrajin Batik', 'period' => 'Juli 2026', 'basic_salary' => 4500000, 'allowance' => 500000, 'deduction' => 150000, 'total' => 4850000, 'status' => 'paid'],
            ['id' => 2, 'employee' => 'Siti Rahayu',        'position' => 'Desainer',        'period' => 'Juli 2026', 'basic_salary' => 5200000, 'allowance' => 750000, 'deduction' => 200000, 'total' => 5750000, 'status' => 'paid'],
            ['id' => 3, 'employee' => 'Agus Wijaya',        'position' => 'Marketing',       'period' => 'Juli 2026', 'basic_salary' => 4800000, 'allowance' => 600000, 'deduction' => 180000, 'total' => 5220000, 'status' => 'pending'],
            ['id' => 4, 'employee' => 'Dewi Lestari',       'position' => 'Admin',           'period' => 'Juli 2026', 'basic_salary' => 4000000, 'allowance' => 400000, 'deduction' => 120000, 'total' => 4280000, 'status' => 'pending'],
            ['id' => 5, 'employee' => 'Hendra Gunawan',     'position' => 'Pengrajin Batik', 'period' => 'Juni 2026', 'basic_salary' => 4500000, 'allowance' => 500000, 'deduction' => 150000, 'total' => 4850000, 'status' => 'paid'],
            ['id' => 6, 'employee' => 'Rina Marlina',       'position' => 'Quality Control', 'period' => 'Juni 2026', 'basic_salary' => 4200000, 'allowance' => 450000, 'deduction' => 130000, 'total' => 4520000, 'status' => 'paid'],
        ];

        // ===== PERUBAHAN 1: SEEDING SESSION =====
        if (!session()->has('payrolls') && !request()->filled('q')) {
    ...
    <style>
        ...
        .pay-success .icon { width: 20px; height: 20px; }
        .pay-success .message { font-weight: 500; }
        ...
        @media (max-width: 768px) {
            ...
            .pay-search { flex-direction: column; align-items: stretch; gap: 10px; }
            .pay-search form { flex-direction: column; }
            .pay-search-wrap { min-width: 100%; }
            .pay-search-actions { width: 100%; justify-content: flex-end; }
            .pay-card-details { grid-template-columns: 1fr 1fr; }
            .pay-modal-box { padding: 24px 20px; margin: 10px; }
            .pay-modal-actions { flex-direction: column; }
            .pay-modal-actions .btn { width: 100%; }
        }
        ...
        @media (max-width: 640px) {
            ...
            .pay-card-footer { flex-direction: column; gap: 12px; align-items: stretch; }
            .pay-card-footer .actions { justify-content: center; }
            .pay-modal-box { padding: 20px 16px; }
            .pay-modal-box h3 { font-size: 18px; }
            .pay-modal-box .icon-danger { width: 48px; height: 48px; }
            .pay-modal-box .icon-danger svg { width: 24px; height: 24px; }
        }
        ...
        @media (max-width: 380px) {
            ...
            .pay-card-details { grid-template-columns: 1fr; }
        }
    </style>
    ...
    <div class="payroll-wrap">
        ...
        <div class="pay-header animate-in" style="animation-delay: 0.05s;">
            <div class="pay-header-left">
                ...
                <p class="subtitle">
                    ...
                    <strong id="payTotalCount">{{ $payrollsCollection->count() }}</strong> slip gaji
                </p>
            </div>
        ...
        @if(session('error'))
            <div class="pay-success" style="background:var(--danger-soft);border-color:var(--danger);color:var(--danger);">
        ...
        <!-- ===== HERO STATS ===== -->
        <div class="pay-hero animate-in" style="animation-delay: 0.10s;" id="payHeroStats">
            <div class="pay-hero-grid">
                <div class="pay-hero-item">
                    <div class="label">Total Payroll</div>
                    <div class="value purple mono" id="payTotalPayroll">{{ formatAngkaPendek($totalPayroll, $currencySymbol) }}</div>
                    <div class="sub" id="payTotalCountSub">{{ $payrollsCollection->count() }} slip gaji</div>
                </div>
                <div class="pay-hero-item">
                    <div class="label">Sudah Dibayar</div>
                    <div class="value green mono" id="payTotalPaid">{{ formatAngkaPendek($totalPaid, $currencySymbol) }}</div>
                    <div class="sub" id="payPaidCount">{{ $countPaid }} slip gaji</div>
                </div>
                <div class="pay-hero-item">
                    <div class="label">Menunggu Pembayaran</div>
                    <div class="value yellow mono" id="payTotalPending">{{ formatAngkaPendek($totalPending, $currencySymbol) }}</div>
                    <div class="sub" id="payPendingCount">{{ $countPending }} slip gaji</div>
                </div>
                <div class="pay-hero-item">
                    <div class="label">Rata-rata Gaji</div>
                    <div class="value mono" id="payAverage">{{ $payrollsCollection->count() > 0 ? formatAngkaPendek(round($totalPayroll / $payrollsCollection->count()), $currencySymbol) : $currencySymbol . '0' }}</div>
        ...
        <div class="pay-filter animate-in" style="animation-delay: 0.15s;">
            ...
            <span class="info" id="filterInfo">
                Menampilkan <strong id="countDisplay">{{ $payrollsCollection->count() }}</strong> 
                slip gaji
            </span>
        </div>
        ...
        <div class="pay-search animate-in" style="animation-delay: 0.20s;">
            <form id="paySearchForm" onsubmit="return false;">
                <div class="pay-search-wrap">
                    ...
                    <input type="text" id="paySearchInput" placeholder="Cari karyawan, posisi, atau periode..." autocomplete="off">
                </div>
                <div class="pay-search-actions">
                    ...
                    <button type="button" class="pay-btn pay-btn-ghost" id="payResetBtn">
            ...
            <div class="pay-grid" id="payrollGrid">
                @forelse($payrolls as $index => $p)
                    @php
                        ...
                        // ===== PERUBAHAN 2: PAKAI INDEX ARRAY =====
                        $itemId = $index;
                        $period = $p['period'] ?? '';
                    @endphp
                    <div class="pay-card {{ $statusClass }} animate-in payroll-item visible-card" 
                        ...
                        <div class="pay-card-details">
                            <div class="item">
                                ...
                                <div class="val mono">{{ formatAngkaPendek($p['basic_salary'], $currencySymbol) }}</div>
                            </div>
                            <div class="item">
                                <div class="lbl">Tunjangan</div>
                                <div class="val mono">{{ formatAngkaPendek($p['allowance'], $currencySymbol) }}</div>
                            </div>
                            <div class="item">
                                <div class="lbl">Potongan</div>
                                <div class="val mono danger">-{{ formatAngkaPendek($p['deduction'], $currencySymbol) }}</div>
                            </div>
                        </div>
                        ...
                        <div class="pay-card-footer">
                            ...
                            <div class="actions">
                                <!-- ===== PERUBAHAN 3: TOMBOL BAYAR DIHAPUS ===== -->
                                <!-- Tombol Bayar sudah dihapus -->
                                <a href="{{ route('payroll.show', ['index' => $itemId]) }}" class="btn-action show" title="Lihat Detail">
                                    <svg class="icon"><use href="#ic-eye"/></svg>
                                    Slip
                                </a>
                                <a href="{{ route('payroll.edit', ['index' => $itemId]) }}" class="btn-action edit" title="Edit">
                                    <svg class="icon"><use href="#ic-edit"/></svg>
                                    Edit
                                </a>
                                <button type="button" class="btn-action delete" title="Hapus"
                                        onclick="openDeleteModal('{{ $itemId }}', '{{ addslashes($p['employee']) }}')">
    ...
    <div class="pay-modal-overlay" id="deleteModal">
        <div class="pay-modal-box">
            ...
            <h3>Hapus Slip Gaji?</h3>

            <p>
                Anda yakin ingin menghapus slip gaji
                <br>
                <span class="pay-desc-text" id="deleteDesc">-</span>
            </p>
            ...
            <div class="pay-modal-actions">
                ...
                <form id="deleteForm" action="{{ route('payroll.destroy', ['index' => 0]) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
    ...
    <script>
        ...
        function openDeleteModal(id, description) {
            ...
            var url = '{{ route("payroll.destroy", ["index" => 0]) }}';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const items = document.querySelectorAll('.payroll-item');
            const countDisplay = document.getElementById('countDisplay');
            const searchIndicator = document.getElementById('searchIndicator');
            const searchResultCount = document.getElementById('searchResultCount');
            const totalCountEl = document.getElementById('payTotalCount');
            const emptyState = document.getElementById('emptyState');
            const gridWrap = document.getElementById('payGridWrap');
            ...
            function formatNumber(num) {
                if (num >= 1000000000) {
                    ...
                    return Math.round(num / 1000).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") + ' Rb';
                } else {
            ...
            function updateStats(visibleCount) {
                const visibleItems = document.querySelectorAll('.payroll-item.visible-card');
                let total = 0, paid = 0, pending = 0;
                let countPaid = 0, countPending = 0;

                visibleItems.forEach(item => {
                    const basic = parseFloat(item.querySelectorAll('.pay-card-details .item .val')[0]?.textContent?.replace(/[Rp,.]/g, '') || 0);
                    const allowance = parseFloat(item.querySelectorAll('.pay-card-details .item .val')[1]?.textContent?.replace(/[Rp,.]/g, '') || 0);
                    const deduction = parseFloat(item.querySelectorAll('.pay-card-details .item .val.danger')[0]?.textContent?.replace(/[Rp,.-]/g, '') || 0);
                    const totalAmount = basic + allowance - deduction;
                ...
                document.getElementById('payTotalPayroll').textContent = currencySymbol + formatNumber(total);
                document.getElementById('payTotalPaid').textContent = currencySymbol + formatNumber(paid);
                document.getElementById('payTotalPending').textContent = currencySymbol + formatNumber(pending);
                document.getElementById('payTotalCountSub').textContent = visibleCount + ' slip gaji';
                document.getElementById('payPaidCount').textContent = countPaid + ' slip gaji';
                document.getElementById('payPendingCount').textContent = countPending + ' slip gaji';
                
                const avg = visibleCount > 0 ? currencySymbol + formatNumber(Math.round(total / visibleCount)) : currencySymbol + '0';
                document.getElementById('payAverage').textContent = avg;
                ...
                if (emptyState) {
                    if (visibleCount === 0 && items.length > 0) {
                        ...
                        emptyState.querySelector('p').textContent = 'Tidak ditemukan slip gaji yang sesuai dengan filter yang dipilih.';
                        const btn = emptyState.querySelector('.pay-btn');
                        if (btn) btn.style.display = 'none';
                    } else if (visibleCount === 0 && items.length === 0) {
                        emptyState.classList.remove('hidden');
                        emptyState.style.display = 'block';
                        emptyState.querySelector('h3').textContent = 'Belum Ada Slip Gaji';
                        emptyState.querySelector('p').textContent = 'Belum ada slip gaji yang tercatat di sistem.';
                        const btn = emptyState.querySelector('.pay-btn');
            ...
            function filterData() {
                ...
                items.forEach(item => {
                    ...
                    if (matchSearch && matchPeriod) {
                        ...
                        item.classList.remove('visible-card');
                        item.classList.add('hidden-card');
                    }
                });

                if (searchText !== '' || selectedPeriod !== 'all') {
