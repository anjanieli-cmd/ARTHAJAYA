<x-app-layout>
    ...
    <style>
        ...
        @media (max-width: 992px) {
            .pe-form-row-3 { grid-template-columns: 1fr 1fr; }
        }

        @media (max-width: 768px) {
            .pe-form-row { grid-template-columns: 1fr; gap: 0; }
            .pe-form-row-3 { grid-template-columns: 1fr; gap: 0; }
        }
        ...
        @media (max-width: 768px) {
            ...
            .pe-form-actions { flex-direction: column; }
            .pe-form-actions .pe-btn { flex: none; }
        }

        @media (max-width: 640px) {
            .pe-card { padding: 16px; }
            .pe-header h1 { font-size: 20px; }
            .pe-btn { font-size: 12px; padding: 8px 14px; }
            .pe-btn .icon { width: 14px; height: 14px; }
            .pe-summary-total { font-size: 16px; }
        }

        @media (max-width: 380px) {
            .pe-wrap { padding: 0 4px; }
    ...
    <div class="pe-wrap">
        ...
        <div class="pe-header animate-in" style="animation-delay: 0.05s;">
            <div class="pe-header-left">
                ...
                <h1>Edit Payroll</h1>
                <p class="subtitle">
                    Edit slip gaji — <strong>{{ $payroll['employee_name'] ?? 'Karyawan Tidak Diketahui' }}</strong> — {{ $payroll['period'] ?? 'Periode Tidak Diketahui' }}
                </p>
            </div>
        ...
        <!-- ===== FORM - FULL WIDTH ===== -->
        <form action="{{ route('payroll.update', ['index' => $index ?? 0]) }}" method="POST" class="pe-form">
            @csrf
            @method('PUT')
            ...
            <div class="pe-card animate-in" style="animation-delay: 0.10s;">
                ...
                <div class="pe-form-row">
                        ...
                        <select name="period" required>
                            @foreach($months as $month)
                                @php
                                    $year = date('Y');
                                    $value = $month . ' ' . $year;
                                    $selected = (isset($payroll['period']) && $value == $payroll['period']) ? 'selected' : '';
                                @endphp
                    ...
                    <div class="pe-form-group">
                        ...
                        <select name="status" required>
                            <option value="pending" {{ (isset($payroll['status']) && $payroll['status'] == 'pending') ? 'selected' : '' }}>⏳ Pending</option>
                            <option value="paid" {{ (isset($payroll['status']) && $payroll['status'] == 'paid') ? 'selected' : '' }}>✅ Dibayar</option>
                            <option value="cancelled" {{ (isset($payroll['status']) && $payroll['status'] == 'cancelled') ? 'selected' : '' }}>❌ Dibatalkan</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="pe-form-row-3">
                    <div class="pe-form-group">
                        ...
                        <input type="number" name="basic_salary" id="basicSalary" value="{{ $payroll['basic_salary'] ?? 0 }}" placeholder="0" min="0" step="1000" required>
                    </div>
                    <div class="pe-form-group">
                        <label>Tunjangan</label>
                        <input type="number" name="allowance" id="allowance" value="{{ $payroll['allowance'] ?? 0 }}" placeholder="0" min="0" step="1000">
                    </div>
                    <div class="pe-form-group">
                        <label>Potongan</label>
                        <input type="number" name="deduction" id="deduction" value="{{ $payroll['deduction'] ?? 0 }}" placeholder="0" min="0" step="1000">
                    </div>
                </div>
                ...
                <div class="pe-summary">
                    ...
                    <div class="pe-summary-item">
                        ...
                        <span class="value mono" id="summaryAllowance">{{ $currencySymbol }}{{ number_format($payroll['allowance'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="pe-summary-item">
                        <span class="label">Potongan</span>
                        <span class="value mono" id="summaryDeduction">{{ $currencySymbol }}{{ number_format($payroll['deduction'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="pe-summary-total">
                        <span class="label">Total Gaji</span>
                        <span class="value mono" id="summaryTotal">{{ $currencySymbol }}{{ number_format($payroll['total'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                </div>
