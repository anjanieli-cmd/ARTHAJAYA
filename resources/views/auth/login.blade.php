...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .visual-copy h2{ 
    font-size:34px; /* DIPERBESAR dari 28px → 34px */
    line-height:1.2; 
    margin-bottom:8px; 
    text-shadow: 0 2px 16px rgba(0,0,0,0.1);
  }
  .visual-copy p{ 
    font-size:17px; /* DIPERBESAR dari 14px → 17px */
    color:rgba(255,255,255,0.85); 
    text-shadow: 0 1px 8px rgba(0,0,0,0.08);
  }

  .visual-illustration{ flex:1; display:flex; align-items:center; justify-content:center; position:relative; z-index:1; margin-top:18px; min-height:0; }
  /* === DIPERKECIL DARI 330px → 260px === */
  .visual-illustration svg{ width:100%; max-width:260px; height:auto; filter:drop-shadow(0 20px 30px rgba(0,0,0,0.18)); }
  ...
  .auth-form .field-row{ animation: fieldIn .5s cubic-bezier(.16,1,.3,1) .42s both; }
  .auth-form .auth-submit{ animation: fieldIn .5s cubic-bezier(.16,1,.3,1) .46s both; }
  ...
  .role-tab:hover{ color:var(--text); border-color:var(--border-hover); transform: translateY(-2px); }
  ...
  .auth-link{ font-size:12px; color:var(--text-mute); transition:color .2s ease; }
  ...
  @media (max-width: 820px){
    ...
    .visual-illustration svg{ max-width:160px; }
...
<div class="auth-page">
  <div class="auth-shell">
    ...
    <div class="auth-panel">
      ...
      <div class="auth-head">
        <h2>Masuk ke Akun</h2>
        <p>Pilih peranmu lalu masukkan kredensialmu.</p>
      </div>
      ...
      <form class="auth-form" method="POST" action="{{ route('login') }}">
        @csrf
        <input type="hidden" name="intended_role" id="intendedRoleInput" value="user">
        <label class="field">
          <span>Email</span>
  ...
  (function(){
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        hiddenInput.value = tab.getAttribute('data-role');
      });
    });
  })();

  // RIPPLE MICRO-INTERACTION (visual only, does not affect click/submit logic)
  (function(){
