...
<style>
    ...
    .panel-icon{ width:38px; height:38px; border-radius:10px; background:rgba(var(--warning-rgb),0.12); color:var(--warning); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
...
<div class="page-head">
    ...
    <p>Perbarui data pos "{{ $item->name }}".</p>
</div>

<form method="POST" action="{{ route('neraca.update', $item) }}">
    @csrf
    @method('PUT')
    <div class="panel">
        <div class="panel-icon-row">
            <div class="panel-icon"><svg class="icon"><use href="#ic-edit"/></svg></div>
            <span>Perubahan akan langsung memengaruhi neraca per {{ $item->as_of_date->translatedFormat('d F Y') }}.</span>
        </div>
        @include('neraca._form')
        <div class="panel-actions">
            <button type="button" class="btn btn-danger-ghost" onclick="document.getElementById('deleteFormNeraca').submit()">Hapus Pos</button>
            <div style="display:flex; gap:10px;">
                <a href="{{ route('neraca.index') }}" class="btn btn-outline">Batal</a>
...
<form method="POST" action="{{ route('neraca.destroy', $item) }}" id="deleteFormNeraca" onsubmit="return confirm('Hapus pos neraca ini?')" style="display:none;">
    @csrf
    @method('DELETE')
</form>
</x-app-layout>