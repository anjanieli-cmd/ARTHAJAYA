<x-admin-layout>
    ...
    <div class="page-head animate-in" style="animation-delay:.03s;">
        ...
        <p>Ubah detail paket <strong>{{ $plan->name }}</strong> — perubahan akan langsung berlaku untuk perusahaan yang menggunakan paket ini.</p>
    </div>

    <div class="form-card animate-in" style="animation-delay:.08s;">
        <div class="form-card-header">
            <h2>Informasi Paket</h2>
            <p>Atur nama, harga, dan fitur dasar paket langganan.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.subscription-plans.update', $plan) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" class="form-control" value="{{ old('name', $plan->name) }}" placeholder="contoh: Pro, Enterprise, Ultimate" required>
                @error('name')
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control" placeholder="Jelaskan fitur dan keunggulan paket ini...">{{ old('description', $plan->description) }}</textarea>
                @error('description')
                    <div class="form-error">
                        <svg><use href="#ic-alert-circle"/></svg>
                        {{ $message }}
            ...
            <div class="grid-3">
                <div class="form-group">
                    ...
                    <input type="number" name="price" class="form-control" value="{{ old('price', $plan->price) }}" min="0" required placeholder="0">
                    @error('price')
                        <div class="form-error">
                            <svg><use href="#ic-alert-circle"/></svg>
                            {{ $message }}
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="max_users" class="form-control" value="{{ old('max_users', $plan->max_users) }}" min="1" placeholder="tak terbatas">
                    @error('max_users')
                        <div class="form-error">
                            <svg><use href="#ic-alert-circle"/></svg>
                            {{ $message }}
            ...
            <div class="form-group">
                <div class="toggle-row">
                    <input type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active', $plan->is_active) ? 'checked' : '' }}>
                    <label for="isActive">Paket aktif</label>
                    <span class="toggle-hint">✓ Perusahaan bisa memilih paket ini saat berlangganan</span>
                </div>
                @error('is_active')
                    <div class="form-error">
                        <svg><use href="#ic-alert-circle"/></svg>
